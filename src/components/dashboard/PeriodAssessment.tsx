...
export function PeriodAssessment({ label, summary, locale = "vi" }: { label: string; summary: PeriodHealthSummary | null; locale?: AppLocale }) {
  ...
  return <section className="card card-hover overflow-hidden">
    <div className="border-b border-slate-100 bg-gradient-to-r from-cyan-50/80 to-white p-4 sm:p-6">
      <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="eyebrow">{label}</p><h2 className="mt-2 text-lg font-extrabold sm:text-xl">{text(locale,`Đánh giá từ ${summary.count} lần đo`,`Assessment based on ${summary.count} readings`)}</h2></div><CategoryBadge category={summary.worstCategory} locale={locale}/></div>
    ...
    <div className="space-y-4 p-4 sm:p-6">
      ...
      <div className="grid gap-3 text-sm min-[360px]:grid-cols-2"><div className="rounded-xl bg-slate-50 p-3"><span className="text-slate-500">{text(locale,"Dao động SYS/DIA","SYS/DIA variability")}</span><b className="mt-1 block">±{summary.variability.systolic} / ±{summary.variability.diastolic}</b></div><div className="rounded-xl bg-slate-50 p-3"><span className="text-slate-500">{text(locale,"Lần đo nguy hiểm","High-risk readings")}</span><b className="mt-1 flex items-center gap-1"><AlertTriangle size={15} className="text-rose-600"/>{summary.highRiskCount}</b></div></div>
    </div>
  </section>;
}
