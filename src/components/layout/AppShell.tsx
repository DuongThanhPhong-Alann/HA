...
export async function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const profile = profileData as { full_name?: string; avatar_preset?: string | null; avatar_path?: string | null } | null;
    ...
    <aside className="relative hidden overflow-hidden border-r border-white/10 bg-[#12122f] p-6 text-white shadow-[12px_0_40px_rgba(31,24,83,.12)] md:flex md:flex-col">
      ...
      <div className="relative mt-auto border-t border-white/10 pt-4">
        <Link href="/profile" className="mb-2 flex min-w-0 items-center gap-3 rounded-xl bg-white/5 p-3 transition hover:bg-white/10">
          ...
          <span className="min-w-0"><b className="block truncate text-xs">{profile?.full_name || "Hồ sơ của tôi"}</b><small className="block truncate text-[10px] text-indigo-100/50">{user?.email}</small></span>
        </Link>
        <div className="[&_button]:text-indigo-100/70 [&_button:hover]:bg-white/10 [&_button:hover]:text-white"><LogoutButton /></div>
      </div>
    </aside>
